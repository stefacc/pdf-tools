...
Running the script with the input file will generate only an INPUT-FILE-NAME-compressed.pdf file
with level 3 compression, the original file will not be deleted.
...
def compress(input_file_path, output_file_path, power=0, gray=False):
    ...
    print(initial_size, input_file_path)
    gs = [
            gs,
            "-sDEVICE=pdfwrite",
    ...
    if gray:
        gs.append("-sColorConversionStrategy=Gray")
        gs.append("-dProcessColorModel=/DeviceGray")
    gs.append(input_file_path)
    subprocess.call(gs)
    final_size = os.path.getsize(output_file_path)
    print(final_size, output_file_path)
...
def main():
    ...
    parser.add_argument(
        "-g", "--gray", action="store_true", help="Move to gray"
    )
    parser.add_argument(
    ...
    if not args.out:
        dirname, extension = os.path.splitext(args.input)
        args.out = dirname + "-compressed" + extension
        print(args.out)

    # Run
    compress(args.input, args.out, power=args.compress, gray=args.gray)
